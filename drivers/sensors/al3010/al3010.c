...
static ssize_t al3010_store_power_state(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ((strict_strtoul(buf, 10, &val) < 0) || (val > 1))	{
		printk("[als_P01] al3010_store_power_state :  inval power state!!!\n");
		return -EINVAL;
	}
	
	printk("[als_P01] al3010_set_power_state\n");
	if ( g_AlsP01ProbeError == 0 && g_bIsP01Attached )	{

		printk("[als_P01] al3010_store_power_state: %lu\n", val);
		ret = set_als_power_state_of_P01( val );

		return ret ? ret : count;
	}else	{
		if ( g_ASUS_hwID >= A91_SR5 )	{
			printk("[als_P01] early set_power_state: %d\n", (int)val );
			g_HAL_als_switch_on = (int)val;
		}
		else
			printk("[als_P01] set_power_state pad mode not ready\n" );
		return 0;
	}
}
...
static int al3010_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
