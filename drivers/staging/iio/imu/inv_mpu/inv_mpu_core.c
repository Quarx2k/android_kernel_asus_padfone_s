...
static int akm_status = 0;

/* mpu6500 orientation */
static __s8 mpu_orientation_sr1[9] = { 0, 1, 0, -1, 0, 0, 0, 0, 1 };

/* akm09911 orientation */
static __s8 akm_orientation_sr1[9] = { 0, 0, 1, -1, 0, 0, 0, -1, 0 };
...
//ASUS_BSP --- Jiunhau_Wang "[A86][Sensor][NA][ATD] support get accel raw data"
//ASUS_BSP +++ Jiunhau_Wang "[A91][Sensor][NA][Spec] support get e-compass raw data"
...
#define REG_AKM09911_WIA2            0x01
#define REG_AKM09911_MEASURE_DATA    0x11
#define REG_AKM09911_CNTL2           0x31

#define DATA_AKM09911_WIA2       0x05
#define DATA_AKM09911_MODE_SM    0x01
#define DATA_AKM_MODE_SM	 0x01

static ssize_t get_mpu65xx_compass_status(struct device *dev, struct device_attribute *devattr, char *buf)
{
	if (g_ASUS_hwID == A90_EVB0) {
		return sprintf(buf, "Not support A90-FF\n");
	}
	return sprintf(buf, "%d\n",akm_status);
}
DEVICE_ATTR(akm_09911_status, S_IRUGO, get_mpu65xx_compass_status, NULL);

static ssize_t get_mpu65xx_compass_i2c(struct device *dev, struct device_attribute *devattr, char *buf)
{
	struct inv_mpu_iio_s *st = iio_priv(dev_get_drvdata(dev));
	uint8_t buffer[9];
	int result = 0;
	...
	inv_secondary_read(REG_AKM09911_WIA2, 1, &data);
	result = (data != DATA_AKM09911_WIA2) ? (0) : (1);
	mutex_unlock(&st->mutex);
	printk(KERN_INFO "[AKM] ID = %d\n", data);
	
	return sprintf(buf, "%d\n", result);
}
DEVICE_ATTR(akm_09911_i2c, S_IRUGO, get_mpu65xx_compass_i2c, NULL);
...
{	
	...
	if (COMPASS_ID_AK09911 == st->plat_data.sec_slave_id) {
		/* check WIA2 register for device ID of AK09911 */
		result = inv_secondary_read(REG_AKM09911_WIA2, 1, &data);
		if (result)
			return result;
		if (data != DATA_AKM09911_WIA2)
			return -ENXIO;
	}

	/*set AKM to single measure access mode*/
	if (COMPASS_ID_AK09911 == st->plat_data.sec_slave_id) {
		result = inv_secondary_write(REG_AKM09911_CNTL2, DATA_AKM09911_MODE_SM);              
	} else {
		result=inv_secondary_write(REG_AKM_MODE,DATA_AKM_MODE_SM);
	}
#if 0
	counter = 3;//DEF_ST_COMPASS_TRY_TIMES;
	while (counter > 0) {
		msleep(10);
	...
	if (COMPASS_ID_AK09911 == st->plat_data.sec_slave_id) 
		result = inv_secondary_read(REG_AKM09911_MEASURE_DATA,
	  ...
	  for (i = 0; i < 3; i++) {
		...
		if (COMPASS_ID_AK09911 == st->plat_data.sec_slave_id) {
			// adjusting sensitivity for AK09911
			o[i] = (short)(((int)o[i] * (sens[i] + 128)) >> 7);
		} else {
...
DEVICE_ATTR(akm_09911_raw, S_IRUGO, get_mpu65xx_compass_rawdata, NULL);
//ASUS_BSP --- Jiunhau_Wang "[A91][Sensor][NA][Spec] support get e-compass raw data"
...
//ASUS_BSP --- Jiunhau_Wang "[A86][Sensor][NA][ATD] support get accel raw data"
//ASUS_BSP +++ Jiunhau_Wang "[A91][Sensor][NA][Spec] support get e-compass raw data"
	&dev_attr_akm_09911_raw.attr,
	&dev_attr_akm_09911_status.attr,
	&dev_attr_akm_09911_i2c.attr,
//ASUS_BSP --- Jiunhau_Wang "[A91][Sensor][NA][Spec] support get e-compass raw data"
#if CAL_DATA_AUTO_LOAD == 1
...
//ASUS_BSP +++ Jiunhau_Wang "[A91][Sensor][NA][Spec] support akm09911"
		akm_status = 1;
//ASUS_BSP --- Jiunhau_Wang "[A91][Sensor][NA][Spec] support akm09911"
	}
...
static int inv_mpu_gyro_irq_init(struct i2c_client *client, struct inv_mpu_iio_s *st)
{
	...
	printk(KERN_INFO "[MPU6500][QQ] st->irq = %d\n", st->irq);
#if 0
	else
	{
	...
	printk(KERN_INFO "[MPU6500] GPIO %d init complete! %d\n", mpu6500_irq_gpio, st->irq);
#endif
	return 0;
...
static int inv_setting_mpu(void)
{
	if ((g_ASUS_hwID > A90_EVB0) && (g_ASUS_hwID < A91_SR3)) {
		// MPU6500 + AKM09911 (SR1)		
		mpu6500_pdata.sec_slave_type = SECONDARY_SLAVE_TYPE_COMPASS;
		mpu6500_pdata.sec_slave_id = COMPASS_ID_AK09911;
		mpu6500_pdata.secondary_i2c_addr = 0xC;
		
		// akm09911 orientation
		memcpy(mpu6500_pdata.secondary_orientation, akm_orientation_sr1, sizeof(mpu6500_pdata.secondary_orientation));
	}
	
	// mpu6500 orientation
	memcpy(mpu6500_pdata.orientation, mpu_orientation_sr1, sizeof(mpu6500_pdata.orientation));
