...
#define REG_AKM09911_INFO1           0x02
#define REG_AKM09911_INFO2           0x03
#define REG_AKM09911_ST1             0x10
#define REG_AKM09911_MEASURE_DATA    0x11
#define REG_AKM09911_CNTL2           0x31
#define REG_AKM09911_SENSITIVITY     0x60
...
#define DATA_AKM09911_MODE_PD    0x00
#define DATA_AKM09911_MODE_SM    0x01
#define DATA_AKM09911_MODE_ST    0x10
#define DATA_AKM09911_MODE_FR    0x1F
...
static int inv_setup_compass_akm(struct inv_mpu_state *st)
{
	...
	if (COMPASS_ID_AK09911 == st->plat_data.sec_slave_id) {
		...
		if (result)
			return result;
		if (data[0] != DATA_AKM09911_WIA2)
			return -ENXIO;
	}

	/*set AKM to Fuse ROM access mode */
	if (COMPASS_ID_AK09911 == st->plat_data.sec_slave_id) {
		result = inv_secondary_write(REG_AKM09911_CNTL2, DATA_AKM09911_MODE_FR);              
	} else {
	...
	if (COMPASS_ID_AK09911 == st->plat_data.sec_slave_id) {	
		result = inv_secondary_read(REG_AKM09911_SENSITIVITY, THREE_AXIS,
	...
	if (COMPASS_ID_AK09911 == st->plat_data.sec_slave_id) {	
		result = inv_secondary_write(REG_AKM09911_CNTL2, DATA_AKM09911_MODE_PD);
	} else {
	...
	if (result)
		return result;
	/* AKM mode register address is 0x31 (REG_AKM09911_CNTL2) for AK09911*/		
	/* AKM mode register address is 0x0A for AK8975, AK8963*/
	if (COMPASS_ID_AK09911 == st->plat_data.sec_slave_id) {	
		result = inv_i2c_single_write(st, REG_I2C_SLV1_REG, REG_AKM09911_CNTL2);
	} else {
...
static int inv_akm_read_data(struct inv_mpu_state *st, short *o)
{
	...
	if (st->chip_config.from_fifo) {
		...
		// Check bit 0 of status1 is equal to DATA_AKM_DRDY (0x01) or not
...
static int inv_check_akm_self_test(struct inv_mpu_state *st)
{
	...
	if (COMPASS_ID_AK09911 == st->plat_data.sec_slave_id) {
		...
		/* set self test mode */
		result = inv_secondary_write(REG_AKM09911_CNTL2, DATA_AKM09911_MODE_ST);
		if (result)
			goto AKM_fail;
		counter = DEF_ST_COMPASS_TRY_TIMES;
		while (counter > 0) {
			usleep_range(DEF_ST_COMPASS_WAIT_MIN, DEF_ST_COMPASS_WAIT_MAX);
			result = inv_secondary_read(REG_AKM09911_ST1, 1, data);
			if (result)
				goto AKM_fail;
...
static int inv_write_akm_scale(struct inv_mpu_state *st, int data)
{
	...
	// No need to update if device is not AK8963?
...
static int inv_resume_akm(struct inv_mpu_state *st)
{
	...
	if (COMPASS_ID_AK09911 == st->plat_data.sec_slave_id) {
		/* AK09911 status 1 register address is 0x10 */
		result = inv_i2c_single_write(st, REG_I2C_SLV0_REG, REG_AKM09911_INFO2);
	} else {
		/* Ak8975, AK8963 status 1 register address is 0x02 */
