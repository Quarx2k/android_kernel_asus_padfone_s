...
#define AKM_DEBUG_IF			1 // Jiunhau
#define AKM_HAS_RESET			1
#define AKM_INPUT_DEVICE_NAME	"compass"
#define AKM_DRDY_TIMEOUT_MS		100
#define AKM_BASE_NUM			10

// ASUS_BSP +++ Jiunhau_Wang "[A91][Sensor][NA][Spec] Porting 9-axis sensor"
#include <linux/platform_device.h>
#include <linux/of_gpio.h>

#define A91_ECOM_GPIO_IRQ_AKM_EVB    76
#define A91_ECOM_GPIO_IRQ_AKM_READY  81
...
#if AKM_IRQ_EN
static int akm09911_irq_gpio = 0;
#endif
...
static struct akm_compass_data *s_akm = NULL;
...
AKECS_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ECS_IOCTL_GET_DELAY:
		...
		break;
	case ECS_IOCTL_GET_LAYOUT:
// ASUS_BSP +++ Jiunhau_Wang "[A91][Sensor][NA][Spec] Porting AKM layout"
		if(g_ASUS_hwID >= A91_SR3)
			akm->layout = PAT5;
		printk(KERN_INFO "[A91][AKM] hw_ID : %d layout = %c\n", g_ASUS_hwID, akm->layout);
// ASUS_BSP +++ Jiunhau_Wang "[A91][Sensor][NA][Spec] Porting AKM layout"		
...
// ASUS_BSP +++ Jiunhau_Wang "[A91][Sensor][NA][Spec] Porting 9-axis sensor"
	__ATTR(akm_chip_id, 0666, akm_chip_id_show, NULL),
	__ATTR(akm_status, 0666, akm_status_show, NULL),
	__ATTR(akm_09911_raw, 0666, akm_read_raw, NULL),
	__ATTR(akm_i2c, 0666, akm_i2c_show, NULL),
// ASUS_BSP --- Jiunhau_Wang "[A91][Sensor][NA][Spec] Porting 9-axis sensor"
#if AKM_DEBUG_IF
	...
	struct i2c_client *client)
{
	...
	if (akm->sense_info[0] != AKM_WIA_VALE) {
		...
		printk(KERN_INFO "[AKM09911] chip ID = %d\n", akm->sense_info[0]);
...
// ASUS_BSP +++ Jiunhau_Wang "[A91][Sensor][NA][Spec] Porting 9-axis sensor"
#if AKM_IRQ_EN
static int inv_mpu_e_compass_irq_init(struct i2c_client *client)
{
	int result = 0;
	
	akm09911_irq_gpio = of_get_named_gpio_flags(client->dev.of_node, "akm09911,irq-gpio", 0, NULL);
	
	result = gpio_request(akm09911_irq_gpio, "akm09911-irq");
	if (result < 0){
		printk(KERN_INFO "[AKM09911] gpio %d request fail!\n", akm09911_irq_gpio);
		goto init_irq_fail;
	}
	
	result = gpio_direction_input(akm09911_irq_gpio);
	if (result < 0){
		printk(KERN_INFO "[AKM09911] gpio_direction_input fail!\n");
		goto init_irq_fail;
	}
	
	s_akm->irq = gpio_to_irq(akm09911_irq_gpio);
	
	if(s_akm->irq < 0)
	{
		printk(KERN_INFO "[AKM09911] gpio_to_irq fail : %d\n", akm09911_irq_gpio);
		goto init_irq_fail;
	}
	else
	{
#if 0
		result = request_irq(s_akm->irq,
                            akm_compass_irq,
...
#endif
		if (result < 0){
			printk(KERN_INFO"[AKM09911] g_akm09911_irq request_irq() error %d.\n",result);
			goto init_irq_fail;
		}
	}
	printk(KERN_INFO "[AKM09911] GPIO 76 init complete!\n");
	return 0;
...
int akm_compass_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(g_ASUS_hwID < A91_SR3)
	{
		printk(KERN_INFO "[AKM] HW_ID = %x\n", g_ASUS_hwID);
		return 0;
	}
	
	printk(KERN_INFO"[AKM09911] %s +++\n",__FUNCTION__);
...
// ASUS_BSP +++ Jiunhau_Wang "[A91][Sensor][NA][Spec] Porting AKM layout"
        if(g_ASUS_hwID >= A91_SR3)
            s_akm->layout = PAT5;
        printk(KERN_INFO "[A91][AKM] layout = %d\n", s_akm->layout);
// ASUS_BSP --- Jiunhau_Wang "[A91][Sensor][NA][Spec] Porting AKM layout"		
#endif
...
// ASUS_BSP --- Jiunhau_Wang "[A91][Sensor][NA][Spec] Porting 9-axis sensor"
#if AKM_IRQ_EN
	inv_mpu_e_compass_irq_init(client); // Jiunhau
#endif
	//s_akm->irq = g_akm09911_irq;
...
{
	pr_info("AKM compass drifine_initcall("7", fn)ver: release.");
