...
ssize_t mydp_write_proc(struct file *filp, const char *buff, size_t len, loff_t *off)
{
	char messages[256];
...
static struct file_operations key_proc_ops = {
	.write = mydp_write_proc,
};

static void create_MYDP_proc_file(void)
{
	struct proc_dir_entry *mydp_proc_file = proc_create(MYDP_PROC, 0, NULL, &key_proc_ops);
...
void sp_hardware_1V_Ctrl(int stat, struct anx7808_platform_data *pdata)
{
	...
	if(stat) {
		ret = regulator_enable(pdata->sp_regulator);
		if (!ret)
			ASUS_DEV_INFO("1V is enabled\n");
	}
	else{
		ret = regulator_disable(pdata->sp_regulator);
		if (!ret)
