...
typedef void (*mydp_func)( const char *msg, int index);

struct mydp_command{
	char *id; 
	int len;
	const mydp_func func;
};
...
void hdmi_asus_rx_initialization(void); //ASUS BSP wei +++
//ANX : (ver:0.2)	
...
unsigned long hex_to_int(char *src);
void mydp_dymSSC(const char *msg, int index);
#define MYDP_PROC "driver/mydp"
#define DYM_SSC "dym_ssc"

//ASUS BSP Wei ---
void sp_tx_sw_error_power_down(void);

//+++ ASUS BSP Bernard, determine ac charger
