 ...
 ****************************************************************************/
#ifndef tmrUsDelay
#define tmrUsDelay(ulTime)	msleep(ulTime)
#endif
...
{
	I2CDataWrite(SP7K_MSG_VENDREQ_CMD, 0x01);
}
	...
	UINT8 *pLscdq)
{
	...
	if (ucBoot == BOOT_FROM_SPI) {
		...
		ucRet = EXISP_LoadCode(spi,ucFwIdx, is_calibration, pIspFw, pCalibOpt, p3acali, pLsc, pLscdq);
		I2CDataWrite(0x1306, ucFwIdx); /* Set which FW to be loaded */
	}
	printk("@@@ISP FW: Load Code End, sts=%d\n", ucRet);
...
_EXIT_:
	...
	if(pFwInfo->DmemFicdmemSize <= 6*1024) { /* Data size <6K, load all bin file */
		ucRet = EXISP_SPIDataWrite(spi,0/*SPI mode0*/, pIspFw, pFwInfo->DmemFicdmemSize, 0x0800);
		I2CDataWrite(0x1011, 0); /* Set CPU to normal operation */
	}
	else {
		ucRet = EXISP_SPIDataWrite(spi,0/*SPI mode0*/, pIspFw, 6*1024, 0x0800);
		printk("ucRet 1 is %x\n",ucRet);
		I2CDataWrite(0x1011, 0); /* Set CPU to normal operation */
		ucRet = EXISP_SPIDataWrite(spi,0/*SPI mode0*/, pIspFw+(6*1024), pFwInfo->DmemFicdmemSize-(6*1024), 0x0800+(6*1024));
		printk("ucRet 2 is %x\n",ucRet);
	}
	...
	/* Transmit IMEM data */
	ucRet = EXISP_SPIDataWrite(spi,0/*SPI mode0*/, pIspFw, pFwInfo->ImemSize, (320*1024)-pFwInfo->ImemSize);
	...
	UINT8 *pLscdq)
{
	...
	default: /* load calibrated 3ACALI.BIN and calibrated LSC.BIN & LSC_DQ.BIN */
		if (p3acali == NULL) {
			pLoadCodeRet->ret3acli = LOADCODE_3ACALI_FILE_ERR;
        } //ASUS_BSP jim3_lin "Fix loading calibrated data"
	...
	UINT8 *pLscdq)
{
	...
	for (i = 0; i < resNumCnt; i++) {
		...
		//printf("tempSize=0x%02x\n", tempSize);
		if ((tempSize%0x10) != 0) {
			tempSize = ((tempSize+0xF)>>4)<<4;
		}
		//printf("size of IQ BIN files %d: 0x%02x\n", i, tempSize);
		iqSize += tempSize;
	}
	//printf("iqSize=%d\n", iqSize);
	//printf("6.pIspFw =%x\n", pIspFw);
	...
	for (i = 0; i < 3; i++) {
		...
		//printf("0.size of IQ BIN files %d: 0x%02x\n", i, tempSize);
		caliSize += tempSize;
		if (i == 0) {
	...
	//printf("start3acali=%x, size3acali=%d\n", start3acali, size3acali);
	//printf("startLsc=%x, sizeLsc=%d\n", startLsc, sizeLsc);
	//printf("startLscdq=%x, size=%d\n", startLscdq, sizeLscdq);
 ...
 *  Return : status result
 *------------------------------------------------------------------------*/
UINT8 EXISP_SPIDataWrite(struct spi_device *spi,UINT8 ucSPIMode, UINT8 *ucStartAddr, UINT32 ulTransByteCnt, UINT32 ulDmaAddr)
{
	UINT8 ucRet = SUCCESS;
	UINT16 tmrCnt = 0;
	...
	/* FIXME: Enable data transaction */
	spi_test_transfer(spi,ucStartAddr,ulTransByteCnt);
...
UINT8 I2CDataWrite(UINT16 addr, UINT8 val)
{
	UINT8 ret;
       ret = sensor_write_reg(mt9m114_s_ctrl.sensor_i2c_client->client, addr, val);
	//ret = mt9m114_s_ctrl.sensor_i2c_client->i2c_func_tbl->i2c_write(mt9m114_s_ctrl.sensor_i2c_client,addr,val,MSM_CAMERA_I2C_BYTE_DATA);
	if(ret == 0)
		return 0;
	printk("%s : fail ret is %c",__FUNCTION__,ret);
	return ret;
}

UINT8 I2CDataRead(UINT16 addr)
{
	UINT8 ret;
	UINT16 val;
       ret = sensor_read_reg(mt9m114_s_ctrl.sensor_i2c_client->client, addr, &val);
	//ret = mt9m114_s_ctrl.sensor_i2c_client->i2c_func_tbl->i2c_read(mt9m114_s_ctrl.sensor_i2c_client,addr,&val,MSM_CAMERA_I2C_BYTE_DATA);
	if(ret == 0)
		return (u8)val;
	printk("%s: fail ret is %c",__FUNCTION__,ret);
	return -1;
